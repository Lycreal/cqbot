...
class Statistics:
    ...
    def incoming_msg(cls, group_id: int, user_id: int, name: str):
        ...
        cls.SaveData.data_count[group_id][user_id] += 1
        cls.SaveData.data_time[group_id].append(datetime.now())
        cls.SaveData.save()

    @classmethod
    def init(cls, group_id: int, user_id: int, name: str):
        # 初始化group_id和user_id结构
        if group_id not in cls.SaveData.data_time.keys():
            cls.SaveData.data_time.update({group_id: []})

        if group_id not in cls.SaveData.data_count.keys():
            cls.SaveData.data_count.update({group_id: {}})
        if user_id not in cls.SaveData.data_count[group_id].keys() and user_id:
            cls.SaveData.data_count[group_id].update({user_id: 0})

        if group_id not in cls.SaveData.data_name.keys():
            cls.SaveData.data_name.update({group_id: {}})
        cls.SaveData.data_name[group_id].update({user_id: name})

    @classmethod
    def clear(cls):
        for group_id in cls.data_count.keys():
            cls.SaveData.last_day_msg_count.update({group_id: cls.top_talker(group_id)})
        for group_id in cls.data_time.keys():
            cls.SaveData.last_day_msg_time.update({group_id: cls.time_result(group_id)})
        cls.data_count = {}
        cls.data_time = {}

    @classmethod
    def top_talker(cls, group_id: int, number=5):  # <20
        listed = cls.sort_dict(cls.SaveData.data_count[group_id])
        msg = f'今日发言Top{number}:'
        for i in range(number if len(listed) > number else len(listed)):
            user_id = listed[i][0]
            count = listed[i][1]
            msg += f'\n{cls.SaveData.data_name[group_id][user_id]}:{count}'
        return msg

    @classmethod
    def time_result(cls, group_id: int):
        result = [time.hour - 4 if time.hour >= 4 else time.hour + 24 - 4 for time in cls.SaveData.data_time[group_id]]
        a = [0] * 12
        for i in range(12):
...
async def _(session: CommandSession):
    group_id: int = session.ctx['group_id']
    msg = Statistics.SaveData.last_day_msg_count[group_id].replace('今日', '昨日')
    await session.send(msg)
...
async def _(session: CommandSession):
    group_id: int = session.ctx['group_id']
    msg = Statistics.SaveData.last_day_msg_time[group_id].replace('今日', '昨日')
    await session.send(msg)
